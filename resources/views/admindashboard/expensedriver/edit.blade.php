...
<div class="card card-custom">
 ...
 <!--begin::Form-->
<form method="post" action="{{route('expensedriver.update',$exdriver->id)}}">
  ...
  <div class="card-body">
   <div class ="row">
      <div class="form-group col-6">
        ...
        <select name="driver_id" class="form-control selectpicker" required>
            @foreach($drivers as $driver)
            <option value="{{$driver->id}}" @if($exdriver->driver_id == $driver->id) selected @endif>{{$driver->name}}</option>
            @endforeach
        </select>
       @error('name')
   ...
   <div class="form-group col-6">
        ...
        <input type="text" class="form-control " value="{{$exdriver->value}}" name="value" required />
       @error('value')
       <p style="color:red;">{{$message}}</p>
       @enderror
       </div>
