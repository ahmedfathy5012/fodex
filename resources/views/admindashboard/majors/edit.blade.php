...
<form method="post" action="{{route('major.update',$major->id)}}" enctype="multipart/form-data">
  ...
  <div class="row"><div class="image-input image-input-outline image-input-circle" id="kt_image_3">
        <div class="image-input-wrapper mb-5" id="im" style="background-image:url({{asset('uploads/'. $major->image)}})"></div>
   ...
   <div class="form-group col-6">
    ...
    <input type="text" class="form-control " value="{{$major->title}}"  name="title" required="required" />
    @error('title')
    <p style="color:red;">{{ $message }}</p>
 @enderror
   </div>   
   ...
   <div class="form-group col-6">
    ...
    <textarea class="form-control" name="description" rows="6">{{$major->description}}</textarea>
