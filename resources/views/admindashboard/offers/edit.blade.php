...
<form method="post" action="{{route('offers.update',$offer->id)}}" enctype="multipart/form-data">
  ...
  <div class="row">
        ...
        <div class="image-input-wrapper mb-5" id="im4"  style="background-image:url({{asset('uploads/'. $offer->image)}})"></div>
...
<div class="row">
    ...
    required="required" id="zone_id" required="required" data-live-search="true">
          ...
          <option value="{{$zone->id}}"style="padding-left:30px;" @if(in_array($zone->id,$offer->zones->pluck('id')->toArray())) selected @endif>{{$zone->name}}</option>
          @endforeach
        </select>
       @error('zone_id')
       <p style="color:red;">{{$message}}</p>
    ...
    required="required" id="seller_id" required="required" data-live-search="true">
          ...
          <option value="{{$seller->id}}"style="padding-left:30px;" @if($seller->id == $offer->seller_id) selected @endif>{{$seller->name}}</option>
          @endforeach
        </select>
       @error('seller_id')
       <p style="color:red;">{{$message}}</p>
    ...
    <div class="mx-auto">
    <label><input type="checkbox" name="paid" value="1" @if($offer->paid == 1) checked @endif>مدفوع</label>
</div></div>
</div>
