...
<?php
    ...
    if(isset($collect)){
         ...
         $orders = $res->orders->where('status',1)->whereBetween('created_at',[$collect->created_at,now()]);
        $countorders = count($orders);
        $money =array_sum($orders->pluck('priceafterdiscount')->toArray());
        $value = $money * ($res->commission /100) + $collect->money_left;
    
    }else{
        $orders = $res->orders->where('status',1);
        $countorders = count($res->orders->where('status',1));
        $money =array_sum($res->orders->where('status',1)->pluck('priceafterdiscount')->toArray());
        $value = $money * ($res->commission /100);
    }?>
     <a href="{{route('seller.show',$id)}}">
    <i class="fas fa-eye font_icon"></i></a>
    <a href="{{route('sellerworkschedule.index',$id)}}" ><img src="{{asset('workse.png')}}"style="width:25px;height:25px;"></a>
...
<a href="{{route('seller.edit',$id)}}" class="btn btn-sm btn-hover-bg-light m-0 span">
                            ...
                            <div style="cursor:pointer;" onclick="deleteseller({{$id}})" class="btn btn-sm btn-hover-bg-light mr-1 span">
...
<button type="button" class="btn btn-danger btn-sm span" data-toggle="modal" data-target="#myModal{{$id}}">بلوك</button>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <div class="row">
           <input type="button"  onclick="blockseller({{$id}})" class="btn btn-sm btn-primary mr-4" value="حفظ">
       </div>
      </div>
    </div>
  </div>
    ...
    <label>
        ...
        @if($seller->availability == 1 ) checked @endif
         value="1"/>
        <span @if($seller->availability == 1 ) title="un available" @else title="available"  @endif></span>
    </label>
</span>
                            <!--<a href="{{route('itemseller',$id)}}" class="btn btn-success btn-sm">المنتجات</a>-->
                       <a href="{{route('seller_items',$id)}}" class="span" style="background-color: #e6f1fb;
    border-radius: 9px;
    padding: 7px;" title="المنتجات">  <i class="fas fa-pizza-slice"  style="color:red;"></i></a> 
                            <span   class="btn btn-success btn-sm span" style="font-size:11px;color:white;" data-toggle="modal" data-target="#myModale{{$id}}">
    تحصيل </span>
...
<div id="myModale{{$id}}" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          <div >@if(isset($collect)) اخر تحصيل منذ 
     ...
     <div class="col-6">
         ...
         <input type="number" required value="{{$countorders}}"  min="1" name="orders" id="orders{{$id}}" class="form-control">
     </div>
     <div class="col-6">
...
<div class="row">
    <div class="col-3"></div>
     <div class="col-3">
         <input type="button" onclick="addcollection({{$id}})" value="حفظ" class="form-control btn btn-success btn-sm m-4">
     </div>
      </div>
