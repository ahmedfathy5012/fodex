...
<form method="post" action="{{route('updatesellerzones',$sellerzone->id)}}">
  ...
  <div class="card-body">
   <div class ="row">
    ...
    <div class="form-group col-lg-4 col-md-6">
        ...
        <select name="zone_id" class="form-control selectpicker" id="zone" required="required" data-live-search="true">
          ...
          <option value="{{$zone->id}}" @if($sellerzone->zone_id == $zone->id) selected @endif>{{$zone->name}}</option>
          @endforeach
        </select>
       @error('zone_id')
   ...
   <!--     <input type="text" class="form-control " required="required" value="{{$sellerzone->delivery_money}}" name="delivery_money" />-->
