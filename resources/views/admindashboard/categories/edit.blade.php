...
<div class="card card-custom">
    ...
    <div class="card-header">
		<div class="card-title">
			...
			<h3 class="card-label"> تعديل قسم رئيسى    </h3>
...
<form method="post" action="{{route('category.update',$category->id)}}" enctype="multipart/form-data">
  ...
  <div class="row"><div class="image-input image-input-outline image-input-circle" id="kt_image_3">
        <div class="image-input-wrapper mb-5" id="im" style="background-image:url({{asset('uploads/'. $category->image)}})" ></div>
     ...
     @enderror
      ...
      <div class='row'>
        ...
        <select name="major_id" class="form-control selectpicker" id="major" required="required" data-live-search="true">
          @foreach($majors as $major)
          <option value="{{$major->id}}" @if($category->major_id == $major->id) selected @endif>{{$major->title}}</option>
          @endforeach
        </select>
       @error('state_id')
       <p style="color:red;">{{$message}}</p>
   ...
   <div class="form-group col-6">
    ...
    <input type="text" class="form-control " value="{{$category->title}}" name="title"  required="required"/>
    @error('title')
    <p style="color:red;">{{ $message }}</p>
 @enderror
   </div>   
    ...
    <div class="row">
                      <span class="switch btn btns-m switch-outline switch-icon switch-primary d-block mx-auto">
    <label>
        <input type="checkbox"    @if($category->is_subcategory == 1) checked @endif name="is_subcategory"/>
        <span></span>
        اقسام فرعيه
    </label>
</span>
