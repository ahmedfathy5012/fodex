...
@section('content')
    <style>
        ...
        .zone-edit-card .card-header {
            background: #ffffff;
            border-bottom: 1px solid #edf0f5;
            padding: 22px 26px;
        }

        .zone-edit-card .card-title {
            margin: 0;
            display: flex;
            align-items: center;
            gap: 12px;
        ...
        /*.zone-edit-card .card-icon svg path,*/
        /*.zone-edit-card .card-icon svg polygon {*/
        ...
        .zone-edit-page .form-group label {
            font-weight: 700;
            color: #3f4254;
            margin-bottom: 8px;
            font-size: 14px;
        ...
        .zone-edit-page .form-control,
        .zone-edit-page .bootstrap-select > .dropdown-toggle {
            min-height: 44px;
        ...
        .zone-edit-page .form-control:focus,
        .zone-edit-page .bootstrap-select.show > .dropdown-toggle {
            border-color: #3699ff !important;
            box-shadow: 0 0 0 3px rgba(54, 153, 255, 0.12) !important;
        }

        .zone-edit-page .bootstrap-select .filter-option {
            text-align: right !important;
        }

        .zone-edit-page .dropdown-menu {
            border-radius: 12px;
            border: 1px solid #edf0f5;
            box-shadow: 0 10px 26px rgba(0, 0, 0, 0.12);
        }

        .zone-edit-page .text-danger,
        .zone-edit-page p[style*="color:red"] {
    ...
    <div class="zone-edit-page">
        <div class="card card-custom zone-edit-card">
            <div class="card-header">
                ...
                <span class="card-icon">
                    ...
                    <h3 class="card-label">تعديل منطقة</h3>
                </div>
            </div>
            ...
            <form method="post" action="{{ route('zone.update', $zone->id) }}">
                ...
                <div class="card-body zone-edit-body">
                            ...
                            <div class="form-group col-lg-6 col-md-6">
                                ...
                                <select name="country_id"
                                    ...
                                    @foreach($countries as $country)
                                        <option value="{{ $country->id }}" @if($zone->country_id == $country->id) selected @endif>
                                            {{ $country->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-lg-6 col-md-6">
                                ...
                                <select name="state_id"
                                    ...
                                    @foreach($states as $state)
                                        <option value="{{ $state->id }}" @if($zone->state_id == $state->id) selected @endif>
                                            {{ $state->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-lg-6 col-md-6">
                                ...
                                <select name="city_id"
                                    ...
                                    @foreach($cities as $city)
                                        <option value="{{ $city->id }}" @if($zone->city_id == $city->id) selected @endif>
                                            {{ $city->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-6 col-md-6">
                                <div class="zone-automatic-box">
                                    ...
                                    <label>
                                        <input type="checkbox"
                                               name="automatic"
                                               @if($zone->automatic == 1) checked @endif
                                               value="1"/>
                                        <span></span>
                                    </label>
                                </span>
                    ...
                    <div class="zone-section">
                        ...
                        <section id="section1">
                            @foreach($zone->prices as $key => $price)
                                <div class="row zone-price-row" id="remove{{ $price->id }}">
                                    ...
                                    <div class="form-group col-lg-3 col-md-6">
                                        ...
                                        <input type="number"
                                               class="form-control"
                                               value="{{ $price->to_distance }}"
                                               name="to_distance[]"
                                               required="required"/>
                                    </div>

                                    <div class="form-group col-lg-3 col-md-6">
                                        <button class="btn btn-sm zone-remove-price-btn"
                                                type="button"
                                                onclick="removef({{ $price->id }})">
                                            حذف
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div class="zone-section">
                        ...
                        <div class="zone-map-note">
                            ارسم حدود المنطقة أو اضغط على المنطقة الحالية لتعديلها، ثم احفظ الصفحة.
                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        function draw_shape() {
            ...
            var latlngsObj = JSON.parse(area.text);
        ...
        function initialize() {
            var oneArea = @json($zone->area);

            var startLat = oneArea && oneArea.lat ? oneArea.lat : 25.381427;
            var startLng = oneArea && oneArea.lon ? oneArea.lon : 49.582997;
