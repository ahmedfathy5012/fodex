...
<form method="post" action="{{route('zone.update',$zone->id)}}">
  ...
  <div class="card-body">
   ...
   <div class="form-group col-6">
        ...
        <select name="country_id" class="form-control selectpicker" onchange="getstates(this)" required="required" data-live-search="true">
          ...
          <option value="{{$country->id}}" @if($zone->country_id == $country->id) selected @endif>{{$country->name}}</option>
          @endforeach
        </select>
       @error('name')
       ...
       <div class="form-group col-6">
        ...
        <select name="state_id" class="form-control selectpicker" onchange="getcities(this)" id="state" required="required" data-live-search="true">
          @foreach($states as $state)
          <option value="{{$state->id}}" @if($zone->state_id == $state->id) selected @endif>{{$state->name}}</option>
          @endforeach
        </select>
       @error('state_id')
       <p style="color:red;">{{$message}}</p>
       @enderror
        ...
        <select name="city_id" class="form-control selectpicker" id="city" required="required" data-live-search="true">
          @foreach($cities as $city)
          <option value="{{$city->id}}" @if($zone->city_id == $city->id) selected @endif>{{$city->name}}</option>
          @endforeach
        </select>
       @error('city_id')
       <p style="color:red;">{{$message}}</p>
   ...
   <div class="form-group col-6">
        ...
        <input type="text" class="form-control " required="required" value="{{$zone->name}}"name="name" />
       @error('name')
       <p style="color:red;">{{$message}}</p>
       @enderror
       </div>
    ...
    <label>
       ...
       automatic
        ...
        <div class="row" style="position: relative;"  id="remove{{$price->id}}">
  <div class="form-group col-3">
        <label>من  <span class="text-danger">*</span></label>
    <input type="number" class="form-control " required="required" name="from_distance[]"  value="{{$price->from_distance}}"    />
   @error('sizename')
   <p style="color:red;">{{$message}}</p>
   @enderror
  ...
  <div class="form-group col-3">
   ...
   <div class="form-group col-2">
    ...
    <input type="number" class="form-control " value="{{$price->to_distance}}" name="to_distance[]"  required="required"/>
    @error('sizecalory')
    <p style="color:red;">{{ $message }}</p>
 @enderror
   </div>   
   ...
   <div class="form-group col-1 mt-4">
        <button class="btn btn-sm  font-weight-bolder btn-light-danger" type="button" onclick="removef({{$price->id}})">حذف</button>
        </div>
   ...
   var one_area = <?php
    ...
    function initialize() {
        ...
        map = new google.maps.Map(document.getElementById('map'), {
            zoom: 12,
            center: new google.maps.LatLng(one_area.lat, one_area.lon)
        });
