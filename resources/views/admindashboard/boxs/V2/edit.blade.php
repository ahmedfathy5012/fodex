...
<div class="card card-custom">
    ...
    <div class="card-header">
		<div class="card-title">
			...
			<h3 class="card-label">   اضافه صندوق </h3>
		</div>
	</div>
 
 
 <!--begin::Form-->
<form method="post" action="{{route('boxs.update',$box->id)}}" enctype="multipart/form-data">

    @csrf
  @method('PUT')
  <div class="card-body">
       <div class="row"><div class="image-input image-input-outline image-input-circle" id="kt_image_3">
        <div class="image-input-wrapper mb-5" id="im" style="background-image:url({{asset('uploads/'. $box->image)}})"></div>
   ...
   <div class ="row">
      <div class="form-group col-6">
        <label>نوع حاله الصندوق <span class="text-danger">*</span></label>
        <select name="boxstatus_id" class="form-control selectpicker"  required>
                        <option value="0" disabled="disabled" selected>الكل</option>
            @foreach($boxstatus as $boxstat)
            <option value="{{$boxstat->id}}" @if($box->boxstatus_id == $boxstat->id) selected @endif>{{$boxstat->title}}</option>
            @endforeach
        </select>
        ...
        <input type="text" class="form-control" id="title" name="title" value="{{$box->title}}"  required />
       ...
       @enderror
        ...
        <input type="text" class="form-control" id="code" name="code" value="{{$box->code}}"  required />
       ...
       @enderror
        ...
        <input type="number" class="form-control" id="height" name="height" value="{{$box->height}}" min="0"  required />
       ...
       @enderror
        ...
        <input type="number" class="form-control" id="width" name="width" value="{{$box->width}}" min="0"  required />
