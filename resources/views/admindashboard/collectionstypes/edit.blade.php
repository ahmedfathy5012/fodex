...
<div class="card card-custom">
    ...
    <div class="card-header">
		<div class="card-title">
			...
			<h3 class="card-label">   اضافه نوع مصروف</h3>
		</div>
	</div>
...
<form method="post" action="{{route('collectionstypes.update',$collect->id)}}">
  ...
  <div class="card-body">
   ...
   <div class="form-group col-6">
        ...
        <input type="text" class="form-control " value="{{$collect->name}}" name="name" required />
       @error('name')
       <p style="color:red;">{{$message}}</p>
       @enderror
       </div> <div class="form-group col-6">
        <label>القيمه <span class="text-danger">*</span></label>
        <input type="number" class="form-control " value="{{$collect->value}}"  name="value" required />
       @error('value')
       <p style="color:red;">{{$message}}</p>
       @enderror
       </div>
