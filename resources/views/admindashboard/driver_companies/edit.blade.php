...
<form method="post" action="{{route('driver_companies.update',$driver->id)}}" enctype="multipart/form-data">
 ...
 @enderror
    ...
    <input type="number" class="form-control " value="{{$driver->commission}}" name="commission"  />
    @error('commission')
    <p style="color:red;">{{ $message }}</p>
 @enderror
   </div>  
 ...
 @enderror
  ...
  <div class="form-group col-6">
        ...
        <select name="zone_ids[]" class="form-control selectpicker"  multiple  data-live-search="true">
          @foreach($zones as $zone)
          <option value="{{$zone->id}}" @if(in_array($zone->id,$driver->zones->pluck("id")->toArray())) selected @endif>{{$zone->name}}</option>
          @endforeach
        </select>
       @error('zone_ids')
       <p style="color:red;">{{$message}}</p>
