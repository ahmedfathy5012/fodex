...
<div class="card card-custom">
    ...
    <div class="card-header">
		<div class="card-title">
			...
			<h3 class="card-label">   اضافه قسم فرعى  </h3>
...
<form method="post" action="{{route('subcategory.update',$subcategory->id)}}" enctype="multipart/form-data">
  ...
  <div class="row"><div class="image-input image-input-outline image-input-circle" id="kt_image_3">
        <div class="image-input-wrapper mb-5" id="im" style="background-image:url({{asset('uploads/'. $subcategory->image)}})"></div>
     ...
     @enderror
      ...
      <div class='row'>
       ...
       <!--   <option value="{{$major->id}}"  @if($subcategory->major_id == $major->id) selected @endif>{{$major->title}}</option>-->
       <!--   @endforeach-->
       <!-- </select>-->
       <!--@error('state_id')-->
       <!--<p style="color:red;">{{$message}}</p>-->
       ...
       <div class="form-group col-6">
        <label>القسم الرئيسى <span class="text-danger">*</span></label>
        <select name="category_id" class="form-control selectpicker" id="category" required="required" data-live-search="true">
   ...
   <div class="form-group col-6">
    ...
    <input type="text" class="form-control " value="{{$subcategory->title}}" name="title"  required="required"/>
    @error('title')
    <p style="color:red;">{{ $message }}</p>
 @enderror
   </div>   
  ...
  <!--  <textarea class="form-control" name="description" rows="6">{{$subcategory->description}}</textarea>-->
