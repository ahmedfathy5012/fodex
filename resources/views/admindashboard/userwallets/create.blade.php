...
<div class="card card-custom">
    ...
    <div class="card-header">
		<div class="card-title">
			...
			<h3 class="card-label">  اضافه مبلغ فى المحفظه</h3>
		</div>
	</div>
 
 
 <!--begin::Form-->
<form method="post" action="{{route('storeuserwallet')}}">
  ...
  <div class="card-body">
   <div class ="row">
        ...
        <select name="user_id" class="form-control selectpicker"  title="&nbsp;"  required="required" data-live-search="true">
          @foreach($users as $user)
          <option value="{{$user->id}}">{{$user->user_code}} -{{$user->phone}}</option>
          @endforeach
        </select>
       @error('user_id')
      ...
      <div class="form-group col-6">
        ...
        <input type="number" class="form-control " min="0" value="{{old('value')}}" name="value" required />
       @error('value')
       <p style="color:red;">{{$message}}</p>
       @enderror
       </div>
