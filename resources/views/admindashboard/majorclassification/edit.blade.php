...
<div class="card card-custom">
 ...
 <!--begin::Form-->
<form method="post" action="{{route('majorclassification.update',$home->id)}}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
  <div class="card-body">
 ...
 @enderror
   ...
   <div class="form-group col-6">
    ...
    required="required" id="major_id" required="required" data-live-search="true">
          ...
          <option value="{{$major->id}}"style="padding-left:30px;" @if($home->major_id == $major->id) selcted @endif>{{$major->title}}</option>
          @endforeach
        </select>
       @error('major_id')
       <p style="color:red;">{{$message}}</p>
...
<div class="form-group col-6">
    ...
    required="required" id="zone_id" required="required" data-live-search="true">
          ...
          <option value="{{$zone->id}}"style="padding-left:30px;" @if(in_array($zone->id,$home->zones->pluck('id')->toArray())) selected @endif>{{$zone->name}}</option>
          @endforeach
        </select>
       @error('zone_id')
       <p style="color:red;">{{$message}}</p>
