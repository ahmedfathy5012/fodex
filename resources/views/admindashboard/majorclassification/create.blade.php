...
<div class="card card-custom">
    ...
    <div class="card-header">
		<div class="card-title">
			...
			<h3 class="card-label"> add majorclassification     </h3>
...
<form method="post" action="{{route('majorclassification.store')}}" enctype="multipart/form-data">
  ...
  <div class="row"><div class="image-input image-input-outline image-input-circle" id="kt_image_3">
   ...
   <div class="form-group col-6">
    ...
    <input type="text" class="form-control " value="{{old('title')}}" name="title"  required="required"/>
    @error('title')
    <p style="color:red;">{{ $message }}</p>
 @enderror
   </div>   
   <div class="form-group col-6">
    <label>القسم الرئيسى    <span class="text-danger">*</span></label>
      <select name="major_id" class="form-control selectpicker"
    required="required" id="major_id" required="required" data-live-search="true">
          @foreach($majors as $major)
          <option value="{{$major->id}}"style="padding-left:30px;">{{$major->title}}</option>
          @endforeach
        </select>
       @error('major_id')
       <p style="color:red;">{{$message}}</p>
   ...
   <div class="form-group col-6">
    ...
    required="required" id="seller_id" required="required" data-live-search="true">
          ...
          <option value="{{$seller->id}}"style="padding-left:30px;">{{$seller->name}}</option>
          @endforeach
        </select>
       @error('seller_id')
       <p style="color:red;">{{$message}}</p>
    ...
    <label>المناطق    <span class="text-danger">*</span></label>
      <select name="zone_id[]" class="form-control selectpicker"
      multiple
    required="required" id="zone_id" required="required" data-live-search="true">
          @foreach($zones as $zone)
          <option value="{{$zone->id}}"style="padding-left:30px;">{{$zone->name}}</option>
          @endforeach
        </select>
       @error('zone_id')
       <p style="color:red;">{{$message}}</p>
