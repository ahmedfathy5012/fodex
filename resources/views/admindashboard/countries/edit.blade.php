...
<div class="card card-custom">
    ...
    <div class="card-header">
		<div class="card-title">
			...
			<h3 class="card-label">   تعديل دوله</h3>
		</div>
	</div>
  ...
  <div class="card-body">
   ...
   <div class="form-group col-6">
       ...
       @enderror
        ...
        <select name="coin_id" class="form-control selectpicker"  required="required" data-live-search="true">
          @foreach($coins as $coin)
          <option value="{{$coin->id}}" @if($country->coin_id == $coin->id) selected @endif>{{$coin->title}}</option>
          @endforeach
        </select>
       @error('coin_id')
       <p style="color:red;">{{$message}}</p>
