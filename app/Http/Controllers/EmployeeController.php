...
class EmployeeController extends Controller
{
    ...
    private function employeeView(string $page): string
    {
        return env('APP_ENV') == 'production'
            ? "admindashboard.employees.$page"
            : "admindashboard.employees.V2.$page";
    }
    ...
    public function index(EmployeeDataTable $dataTable)
    {
        ...
        return $dataTable->render($this->employeeView('index'), [
            'countries' => $countries,
            'states' => $states,
            'cities' => $cities,
            'zones' => $zones
    ...
    public function create()
    {
        ...
        return view($this->employeeView('create'))
    ...
    public function show(EmployeescontractDataTable $dataTable, $id)
    {
        ...
        $contract = Employeescontract::where('employee_id', $id)
            ->where("active", 1)
            ->latest()
            ->first();

        return $dataTable->render($this->employeeView('show'), [
            'employee' => $employee,
            'contract' => $contract
        ]);
    }
    ...
    public function edit($id)
    {
        ...
        return view($this->employeeView('edit'))
            ->with('genders', $genders)
            ->with('roles', $roles)
            ->with('armycases', $armycases)
            ->with('statuss', $statuss)
    ...
    public function employeecontracts(EmployeescontractDataTable $dataTable, $id)
    {
        ...
        return $dataTable->render($this->employeeView('contracts'), [
            'id' => $id
        ]);
    }
    ...
    {
        return view($this->employeeView('addemployeecontract'))
            ->with('id', $id);
    }

    public function storeemployeecontract(Request $request, $id)
    {
        Employeescontract::where('employee_id', $id)->update([
            'active' => 0
        ]);
    ...
    public function editemployeecontract($id)
    {
        ...
        return view($this->employeeView('editemployeecontract'))
            ->with('contract', $contract);
    }
    ...
    public function employeeawards(AwardDataTable $dataTable, $id)
    {
        ...
        return $dataTable->render($this->employeeView('employeeawards'));
    }
    ...
    public function employeediscounts(DiscountDataTable $dataTable, $id)
    {
        ...
        return $dataTable->render($this->employeeView('employeediscounts'));
    }
    ...
    public function notcollectemployees(NotcollectemployeeDataTable $dataTable)
    {
        ...
        return $dataTable->render($this->employeeView('notcollectemployee'), [
            'countries' => $countries,
            'states' => $states,
            'cities' => $cities,
            'zones' => $zones
    ...
    public function addemplyeeexpense(Request $request)
    {
        ...
        if ($allcolletions > ($expenses + $request->value)) {
            $expense = ExpenseEmployee::where('employee_id', $request->id)
    ...
    public function countriespermissions($id)
    {
        ...
        return view($this->employeeView('countriespermissions'))
            ->with('countries', $countries)
            ->with('states', $states)
            ->with('cities', $cities)
            ->with('zones', $zones)
    ...
    public function savecountriespermissions(Request $request, $id)
    {
        ...
        if ($request->state_id) {
            $employee->states()->sync($request->state_id);
        } else {
