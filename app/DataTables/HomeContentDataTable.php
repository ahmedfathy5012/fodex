...
class HomeContentDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
->addColumn('image',function(HomeContent $home){
    if($home->image){
            $image = asset("uploads/$home->image");
            return '<img  src="' . $image . '" style ="width:100px;height:100px;" />';
        }
              })->editColumn('title',function(HomeContent $home){

        
            return '<a  href="' . route("sellerscontent",$home->id) . '" />'.$home->title.'</a>';
              })
            ->addColumn('action', 'admindashboard.homecontent.action')
