...
class MajorclassificationcontentSellerDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
 ->eloquent($query)
 ->editColumn('major_id',function(MajorclassificationcontentSeller $seller){
                if($seller->seller->major){
                    return $seller->seller->major->title;
                }
            })
             ->editColumn('name',function(MajorclassificationcontentSeller $seller){
             //   if($seller->seller){
                    return $seller->seller->name;
              //  }
            })->editColumn('phone',function(MajorclassificationcontentSeller $seller){
             //   if($seller->seller){
     ...
     */
    public function query(MajorclassificationcontentSeller $model)
    {
  //    $sellers_ids = HomeContent::where('id',$this->id)->first()->sellers;//->orderBy('homecontent_sellers.status','desc');//->pluck('seller_id')->toArray();
     //   $sellers = $model->newQuery()->whereIn('id',$sellers_ids);
        // return $sellers_ids;
   //      $sellers = HomecontentSeller::where('homecontent_id',$this->id)->orderBy('status','desc')->get();
         $sellers = $model->newQuery()->where('majorclassificationcontent_id',$this->id)->orderBy('status','desc');
         return $sellers;
    }
