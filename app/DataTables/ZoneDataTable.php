...
class ZoneDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
                   ->editColumn('country_id',function(Zone $zone){
                if($zone->country){
                    return $zone->country->name;
                }
            }) ->editColumn('state_id',function(Zone $zone){
                if($zone->state){
                    return $zone->state->name;
                }
            })->editColumn('city_id',function(Zone $zone){
                if($zone->city){
                    return $zone->city->name;
                }
            })
            ->addColumn('action', 'admindashboard.zones.V2.action')
