...
class MajorClassificationSelectDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
     ->addColumn('image',function(MajorClassificationSelect $home){
    if($home->image){
            $image = asset("uploads/$home->image");
            return '<img  src="' . $image . '" style ="width:100px;height:100px;" />';
        }
              })
            ->addColumn('action', 'admindashboard.majorclassification.actionselect')
     ...
     */
    public function query(MajorClassificationSelect $model)
    {
